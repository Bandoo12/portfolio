...
const CURVE_P0: Pt = { x: 8.5, y: 436.5 };
const CURVE_P1: Pt = { x: 242, y: 436.5 };
const CURVE_P2: Pt = { x: 553, y: 340.5 };
const CURVE_P3: Pt = { x: 835, y: 159.5 };
const CURVE_BASELINE_Y = 436.5;
...
// Scripted crash points for rounds where the visitor has a bet down — cycles
// through a generous cash-out window, an instant bust, a huge multiplier,
// and mid-range outcomes, so a portfolio visit reliably demonstrates both a
// win and a loss instead of leaving it to chance. Only advances when a bet
// was actually placed (see demoIdxRef), so an idle visitor never burns
// through the cycle.
const DEMO_CRASH_SEQUENCE = [6.2, 1.08, 12.4, 2.35, 1.31, 30.15];
...
function CrashCurve({ t, dead }: { t: number; dead: boolean }) {
  ...
  return (
    <svg viewBox={`0 0 ${ARENA_W} ${ARENA_H}`} width="100%" height="100%" style={{ position: 'absolute', inset: 0 }}>
      <defs>
        <linearGradient id="av-curve-stroke" x1={CURVE_P3.x} y1={CURVE_P3.y} x2={CURVE_P0.x} y2={CURVE_P0.y} gradientUnits="userSpaceOnUse">
...
function EngineFlame({ show }: { show: boolean }) {
  if (!show) return null;
  return (
...
function Rocket({ x, y, deg, opacity, flying }: { x: number; y: number; deg: number; opacity: number; flying: boolean }) {
  return (
    <div style={{
      ...
      <EngineFlame show={opacity > 0.4} />
    </div>
  );
}

const MultiplierReadout = React.memo(function MultiplierReadout({ mult, crashed }: { mult: number; crashed: boolean }) {
  return (
    <div className={inter.className} style={{
      position: 'absolute', left: '42%', top: '46%', transform: 'translate(-50%,-50%)',
      fontWeight: 900, fontSize: 128, lineHeight: 1, color: crashed ? '#FF4D6A' : '#fff',
      textShadow: crashed ? '0 8px 32px rgba(255,0,80,0.4)' : '0 8px 32px rgba(162,0,255,0.33)',
            ...
            <div style={{ width: ARENA_W, display: 'flex', flexDirection: 'column', gap: GAP }}>
              <Panel style={{ width: ARENA_W, height: MAIN_ARENA_H, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ position: 'relative', width: ARENA_W, height: ARENA_H, overflow: 'hidden' }}>
                  ...
                  {phase === 'crashed' && (
                    <div className={inter.className} style={{ position: 'absolute', left: '42%', top: '62%', transform: 'translate(-50%,-50%)', fontWeight: 700, fontSize: 22, color: '#FF4D6A', zIndex: 7 }}>
                      УЛЕТЕЛ!
                    </div>
                  )}
                  <AnimatePresence>
                    {burstKey > 0 && myCashedAt !== null && phase !== 'betting' && (
                      <motion.div key={burstKey} initial={{ opacity: 0, y: 10, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}
                        style={{ position: 'absolute', left: '42%', top: '62%', transform: 'translate(-50%,-50%)', zIndex: 9 }}>

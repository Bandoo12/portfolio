...
function Rocket({ x, y, deg, opacity, flying, outcome }: { x: number; y: number; deg: number; opacity: number; flying: boolean; outcome: RocketOutcome }) {
  return (
    <div style={{
      ...
      // Warm glow matching the curve's own gradient (not a dark drop-shadow)
      // so the sprite reads as part of the same trail instead of a separate
      ...
      transition: flying ? 'none' : 'left 1.15s cubic-bezier(.3,0,.7,1), top 1.15s cubic-bezier(.3,0,.7,1), opacity 1.05s ease-in 0.15s, color 0.3s ease',
    }}>
      <RocketIcon />
    </div>
  );
...
export default function AviatorPage() {
  ...
  // No drift on crash — the sprite stays exactly on the line's frozen tip
  // and only fades/recolors, so it never visibly detaches from the curve.
  const rocketDisplayX = rocketX;
            ...
            <div style={{ width: ARENA_W, display: 'flex', flexDirection: 'column', gap: GAP }}>
              <Panel style={{ width: ARENA_W, height: MAIN_ARENA_H, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ position: 'relative', width: ARENA_W, height: ARENA_H, overflow: 'hidden' }}>
                  ...
                  {phase === 'crashed' && (
                    <div className={inter.className} style={{
                      position: 'absolute', left: '42%', top: '62%', transform: 'translate(-50%,-50%)', fontWeight: 700, fontSize: 22,

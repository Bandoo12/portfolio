...
/* "Нарды 36" (Liga Stavok quick game — 6 sequential dice throws race to a 1-36 cell,
   ...
   v3: history strip shows the deciding (final) throw's dice combo under each past
   result — a subtle, provable-fairness nod to the real game's dice-combo-to-value
   link, without changing the sum-of-6-throws mechanic itself. */
...
function MiniDicePair({ blue, red }: { blue: number; red: number }) {
  return (
    <div style={{ display: 'flex', gap: 2 }}>
      <DiceFace face={blue} color="#2F6BFF" size={13} />
      <DiceFace face={red} color="#FF3860" size={13} />
    </div>
  );
}
...
type HistoryEntry = { n: number; blue: number; red: number };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      while (alive) {
        ...
        const decidingThrow = collected[collected.length - 1];
        setHistory(h => [{ n: landed, blue: decidingThrow.blue, red: decidingThrow.red }, ...h].slice(0, 20));
    ...
    <div style={{ minHeight: '100vh', background: '#050311', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ margin: '4px 14px 0', borderRadius: 26, padding: '14px 10px 10px', position: 'relative', background: 'linear-gradient(180deg, rgba(255,255,255,0.05), rgba(255,255,255,0.01))', border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
          ...
          {/* History strip — each result carries the deciding throw's dice combo */}
          <div className="nz-scroll" style={{ display: 'flex', gap: 5, marginTop: 10, overflowX: 'auto', position: 'relative', zIndex: 2 }}>
            {history.map((h, i) => (
              <div key={i} style={{
                flexShrink: 0, minWidth: 32, padding: '4px 5px 5px', borderRadius: 10,
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3,
                background: i === 0 ? `linear-gradient(135deg, ${NEON.gold}, ${NEON.violet})` : 'rgba(255,255,255,0.06)',
                border: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.08)',
              }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: i === 0 ? '#2a1400' : 'rgba(255,255,255,0.75)' }}>{h.n}</span>
                <MiniDicePair blue={h.blue} red={h.red} />
              </div>
            ))}
          </div>
        </div>
